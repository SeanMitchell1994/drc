...
def run_esn_exp(param):
    ...
    while sub_iterates <= max_sub_iterates:
        ...
        new_rc.Generate_Reservoir()
        new_rc.Train(training_length)
        new_rc.Load_Data('E:\School\Graduate\Research\Code\MATLAB\datasets\lorenz_x_ic_shift.txt')
        new_rc.Run_Predictive(test_length)
        new_rc.Compute_MSE(test_length)
        mse_temp = new_rc.Get_MSE() + mse_temp
        sub_iterates = sub_iterates + 1
...
def main():
    ...
    # two_sigma_dsn = 2*stddev
    # two_sigma_esn = 2*stddev2

    # for i in range(len(mse_list)):
    #     upper_bound.append(mse_list[i] + two_sigma_dsn)
    #     lower_bound.append(mse_list[i] - two_sigma_dsn)
    ...
    #plt.plot( param_list,upper_bound, '--b', linewidth=1)
    #plt.plot( param_list,lower_bound, '--b', linewidth=1)
    plt.plot( param_list_esn,mse_list_esn, 'g', linewidth=1 )
    #plt.plot( param_list,p(param_list),'r--', linewidth=1)
    plt.plot(param_list_esn[mse_list_esn.index(min2)],mse_list_esn[mse_list_esn.index(min2)],'ro', markersize=5) 
    plt.title('Shift Map Parameter Sweep (Lorenz x time series) (IC Shift)')
    ...
    # leg = plt.legend(['DSN MSE','Minimum DSN MSE','Shift Map LE Zero-Crossing','Negative Entropy','Positive Entropy'],bbox_to_anchor=(1,0.895), loc="center left", numpoints=1)
